<template>
  <v-card class="post-card justify-center" color="red" dark>
    <v-toolbar color="red darken-2" dark>
      ...
      <v-menu offset-y>
        <template v-slot:activator="{ on, attrs }">
          <v-btn dark v-on="on" v-bind="attrs" style="margin:2px">
            Categories
          </v-btn>
        </template>
        <v-card>
    ...
    <v-container fluid>
      <v-row dense>
        <v-col v-for="post in posts" :key="post.node.title" :cols="post.flex">
          <v-card>
